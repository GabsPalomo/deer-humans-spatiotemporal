...
deer_jmat <- read_csv('./spatial_analysis/model_matrices/deer_jmat.csv')[, -1]
## deer detections across all sites and seasons
# [-1] is to remove the first column which has site names 
deer_detections <- read_csv('./spatial_analysis/model_matrices/deer_detections.csv')[, -1]

## covariates 
cov_mat <- read_csv('./spatial_analysis/model_matrices/cov_mat.csv')

# indices ----------------------------------------------------------------------
nsites <- nrow(deer_detections)
nseasons <- ncol(deer_detections)
...
# Create data list
my.data <- list(
  count = as.matrix(deer_detections),
  offset = as.matrix(deer_jmat)
)
...
readr::write_rds(mcmc.output, paste0('./spatial_analysis/rds_model/deer_mcmc_output_01_', Sys.Date(), '.rds'))
...
MCMCtrace(mcmc.output, 
          ...
          Rhat = TRUE,
          pdf = TRUE, 
          filename = paste0('./spatial_analysis/model_results/deer_mcmc_output_01_', Sys.Date(), '.pdf'))
